...
export const Board = () => {
    ...
    const nextMove = () => {
        const newBoard = [...gameBoardHistory];
        const isBlack = iterator % 2 == 0 ? false : true;
        newBoard.push(makeMove(gameBoard, game, iterator, isBlack));
        setGameBoardHistory(newBoard);
        setGameBoard(gameBoardHistory[gameBoardHistory.length - 1]);
        setIterator(iterator + 1);
    }
    const prevMove = () => {
        setIterator(iterator - 1);
        setGameBoard(gameBoardHistory[iterator]);
        let newTurn = turn;
        setTurn(newTurn);
    }
